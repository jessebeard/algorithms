...
let index = 0
let curRoll = nums[index]
while (winnerIndex.length < boardsArr.length) {
  cards = cards.map(card=> (
    card.map(row => (
      row.map(num => num === curRoll ? -1: num)
    ))
...
const bingoBoard = cards[winnerIndex[winnerIndex.length - 1]]
console.log(bingoBoard, winnerIndex[winnerIndex.length - 1])
let total = 0
for(let x = 0; x < 5; x++){
  for(let y = 0; y < 5; y++){
    if (bingoBoard[y][x] !== -1){
